...
contract('App', function ([_, owner, otherAdmin]) {
  ...
  beforeEach('deploying dependency', async function () {
    this.dependency = await ImplementationDirectory.new({ from: owner })
  });
  ...
  describe('setDependency', function () {
    beforeEach('setting a dependency', async function () {
      await this.app.setDependency(depName, this.dependency.address);
    });

    it('should retrieve the dependency', async function () {
      const dep = await this.app.getDependency(depName);
      dep.should.be.eq(this.dependency.address);
    });

    it('should tell whether a dependency is set', async function () {
      (await this.app.hasDependency(depName)).should.be.true
    })
  });
